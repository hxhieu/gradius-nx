...
void
GameStateManager::handleEvent(const SDL_Event& event)
{
	switch (event.type) {
...
bool
GameStateManager::handleGame(flat2d::GameData* gameData)
{
...
void
GameStateManager::loadGame(flat2d::GameData* gameData)
{
	flat2d::EntityContainer* entityContainer = gameData->getEntityContainer();
	entityContainer->registerObject(new Player(200, 200));

	// You can register your objects to the container here. Objects extend the
	// Entity class in flat
}

void
GameStateManager::loadSplash(flat2d::GameData* gameData)
{
	// TODO: Splash screen
}

void
GameStateManager::run(flat2d::GameData* gameData)
{
	switch (currentState) {
		case SPLASH:
			loadSplash(gameData);
